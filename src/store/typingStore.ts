...
export const useTypingStore = create<TypingState & TypingActions>(
  (set, get) => ({
    ...
    typeCharacter: (character) => {
      const state = get();
      if (!state.isActive || state.currentPosition >= state.currentText.length)
        return;
      ...
      set((prevState) => {
        const newState = {
          ...
          correctPosition: isCorrect
            ? prevState.correctPosition + 1
            : prevState.correctPosition,
          errors: isCorrect ? prevState.errors : prevState.errors + 1,
        };

        // Calculate metrics
        const timeElapsed = prevState.startTime
          ? (Date.now() - prevState.startTime.getTime()) / 1000 / 60
          : 0;

        if (timeElapsed > 0) {
          const wordsTyped = newState.correctPosition / 5; // Average word length
          newState.currentSpeed = Math.round(wordsTyped / timeElapsed);
          newState.currentAccuracy =
            newState.totalKeystrokes > 0
              ? Math.round(
                  (newState.correctPosition / newState.totalKeystrokes) * 100,
                )
              : 100;
        }
      ...
      if (timeElapsed > 0) {
        ...
        const accuracy =
          state.totalKeystrokes > 0
            ? Math.round((state.correctPosition / state.totalKeystrokes) * 100)
            : 100;
