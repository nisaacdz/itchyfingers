...
export const useTournamentRealtime = ({
  ...
  const [state, setState] = useState<TournamentRealtimeState>(initialRealtimeState);
  ...
  const updateState = useCallback((updates: Partial<TournamentRealtimeState>) => {
  ...
  const deriveCurrentGamePhase = useCallback(
    (
      ...
      if (currentSocketStatus === 'idle' || currentSocketStatus === 'connecting') {
      ...
      if (session.started_at) {
        const userSession = authClientIdFromHook ? participantsList[authClientIdFromHook] : null;
        if (userSession?.ended_at || (session.text && userSession?.correct_position === session.text.length)) {
  ...
  useEffect(() => {
    ...
    isStaticLoading, staticError,
    isAuthLoading, currentAuthClient, // Auth related
    state.socketStatus, state.tournamentSession, state.participants, // Realtime state
    deriveCurrentGamePhase, updateState, state.gamePhase // Hook utilities
  ]);
  ...
  useEffect(() => {
    ...
    if (isStaticLoading || staticError || isAuthLoading || !currentAuthClient || !tournamentId || !initialTournamentStaticInfo) {
      // If any pre-condition fails after initial loading phase, set appropriate error phase.
      if (!isStaticLoading && staticError) updateState({ gamePhase: 'error_static_data', lastSocketError: staticError });
      else if (!isAuthLoading && !currentAuthClient) updateState({ gamePhase: 'error_auth', lastSocketError: "User information is not available." });
    ...
    socketService.connect(tournamentId, token)
      ...
      .catch((error: Error) => {
        console.error("useTournamentRealtime: Failed to connect socket:", error);
        updateState({
          socketStatus: 'error',
          lastSocketError: error.message || 'Failed to connect to tournament.',
          gamePhase: 'error_socket',
        });
        toast({ title: "Connection Error", description: "Could not connect to the tournament server.", variant: "destructive" });
      });
    ...
    const handleTournamentStart = (sessionData: TournamentSession) => {
      ...
      setState(prev => ({
        ...
        gamePhase: deriveCurrentGamePhase('connected', sessionData, prev.participants, currentAuthClient.id),
        lastSocketError: null,
      }));
      if (newTypingText) {
    ...
    const handleTournamentUpdate = (updateData: TournamentUpdateSchema) => {
      ...
      const currentUserSession = currentAuthClient ? newParticipantsMap[currentAuthClient.id] : null;
      ...
      setState(prev => ({
        ...
        typingText: newTypingText,
        gamePhase: deriveCurrentGamePhase('connected', updateData.tournament, newParticipantsMap, currentAuthClient.id),
        lastSocketError: null,
      }));
    };
      ...
      if (payload.success && payload.data) {
        ...
        setState(prev => {
          ...
          return {
            ...
            gamePhase: deriveCurrentGamePhase(prev.socketStatus, prev.tournamentSession, newParticipants, currentAuthClient.id)
          };
        });
      }
    };
    ...
    const handleDisconnect = (reason: Socket.DisconnectReason) => {
      ...
      const isHardDisconnect = reason === "io server disconnect" || reason === "io client disconnect";
      updateState({
    ...
    return () => {
      ...
      console.log("useTournamentRealtime: Cleaning up socket listeners and disconnecting.");
    ...
    initialTournamentStaticInfo,
    isStaticLoading, staticError,
    isAuthLoading, currentAuthClient,
    state.gamePhase,
    updateState, toast, navigate, deriveCurrentGamePhase, state.typingText // Utilities and state dependencies
  ]);

  // Actions to be called by components
  const sendTypeCharacter = useCallback((character: string) => {
  ...
  const sendLeaveTournament = useCallback(() => {
    if (state.socketStatus === 'connected') {
      socketService.emitLeaveTournament();
        ...
        toast({title: "Not Connected", description: "Cannot send leave request.", variant: "destructive"});
        navigate('/tournaments');
    }
