  ...
  return (
    <main className="flex-grow grid grid-cols-1 md:grid-cols-12 gap-4 p-3 md:p-4 overflow-hidden">
      ...
      <aside className="md:col-span-3 flex flex-col gap-4 overflow-y-auto custom-scrollbar pr-1">
        <div className="bg-muted p-4 rounded-lg shadow-md backdrop-blur-sm">
          {statsSlot}
        </div>
        <div className="bg-muted p-4 rounded-lg shadow-md backdrop-blur-sm flex-grow min-h-[200px]">
          {participantsSlot}
        </div>
      </aside>

      {/* Right Column: Main Typing Challenge Area */}
      <section className="md:col-span-9 bg-muted p-4 md:p-6 rounded-lg shadow-xl backdrop-blur-md flex flex-col items-center justify-center overflow-hidden">
        {mainChallengeSlot}
      </section>
    </main>
  );
