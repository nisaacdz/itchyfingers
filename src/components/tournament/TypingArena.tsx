...
import { computeAbsolutePosition, createWhiteSpaceErrorHighlightStyle } from '@/lib/typing';
...
const getParagraphMetrics = (paragraphEl: HTMLParagraphElement | null): { fontSize: number; lineHeight: number; charWidth: number } => {
...
export const TypingArena = ({
  ...
  const [metrics, setMetrics] = useState({ fontSize: 24, lineHeight: 36, charWidth: 14.4 }); // Initial reasonable defaults
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === ' ' || event.key === 'Backspace' || (event.key.length === 1 && !event.ctrlKey && !event.metaKey && !event.altKey) ) {
  ...
  const caretPositions = useMemo(() => {
    ...
    if (paragraphRef.current) {
      Object.values(allParticipants).forEach(p => {
        positions[p.client.id] = computeAbsolutePosition(paragraphRef, p.current_position);
      });
    }
    return positions;
  ...
  const whiteSpaceErrorHighlights = useMemo(() => {
    if (!paragraphRef.current || !currentUserSession || currentUserSession.correct_position === currentUserSession.current_position) {
      return [];
    }
    const highlights = [];
    for (let i = currentUserSession.correct_position; i < currentUserSession.current_position; i++) {
      if (text[i] === ' ') {
        const pos = computeAbsolutePosition(paragraphRef, i);
        highlights.push(
          <div
            key={`ws-error-${i}`}
            style={createWhiteSpaceErrorHighlightStyle(pos, metrics.charWidth, metrics.lineHeight)}
    ...
    return (
        <div className="flex flex-col items-center justify-center text-center h-full p-4">
            <motion.div initial={{scale:0.5, opacity:0}} animate={{scale:1, opacity:1}} transition={{duration:0.5}}>
                ...
                <h3 className="text-3xl font-bold text-slate-100 mb-2">You Finished!</h3>
                <p className="text-slate-300">Great job! Waiting for other racers to complete.</p>
                <p className="text-sm text-slate-400 mt-4">WPM: {Math.round(currentUserSession.current_speed)}, Accuracy: {currentUserSession.current_accuracy.toFixed(1)}%</p>
            </motion.div>
        </div>
    )
  }
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center p-2 md:p-4 focus:outline-none" tabIndex={-1}>
        ...
        {Object.values(allParticipants).map((p) => {
          ...
          return (
            <Caret
              ...
              displayName={p.client.user?.username || `Anon-${p.client.id.substring(0,4)}`}
            />
          );
        })}
      </div>
