...
const Caret: React.FC<{ participant: TypingSessionSchema, text: string, parentRef: React.RefObject<HTMLDivElement>, fontSize: number }> = ({ participant, text, parentRef, fontSize }) => {
  ...
  useEffect(() => {
    if (parentRef.current && text) {
      ...
      if (textNode && textNode.nodeType === Node.TEXT_NODE) {
        ...
        const charPosition = Math.min(participant.current_position, text.length -1);
        ...
        try {
            ...
            setPosition({ top: 0, left: Math.min(participant.current_position, text.length) * (fontSize * 0.6) });
        }
      } else {
  ...
  return (
    <div
      style={{
        ...
        width: '2px',
        height: `${fontSize}px`,
        backgroundColor: participant.client.user?.username === 'You' ? 'blue' : 'gray', // Distinguish user's caret
...
const TypingArea: React.FC<TypingAreaProps> = ({
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (event: KeyboardEvent) => {
      if (disabled || !currentTournament || currentTournament.status !== 'active') return;
  ...
  if (!currentTournament) {
    return <div className="p-4 text-muted-foreground">Loading typing area...</div>;
  }

  const typedText = clientId ? participants.find(p => p.client.id === clientId)?.current_position || 0 : 0;
  const correctTextEnd = clientId ? participants.find(p => p.client.id === clientId)?.correct_position || 0 : 0;
  ...
  return (
    <div
      ...
      style={{ fontFamily: '"Courier Prime", monospace', lineHeight: `${fontSize * 1.5}px` }}
      tabIndex={0} // Make it focusable
    >
      <div ref={textDisplayRef} className="whitespace-pre-wrap break-all" style={{ fontSize: `${fontSize}px` }}>
        {text.split('').map((char, index) => {
          let charColor = 'text-foreground'; // Default
          if (clientId && index < typedText) {
            charColor = index < correctTextEnd ? 'text-green-500' : 'text-red-500';
          }
          return (
            <span key={index} className={charColor}>
              {char === ' ' && index >= correctTextEnd && index < typedText ? <span className="bg-red-200 dark:bg-red-800 rounded-sm"> </span> : char}
            </span>
          );
        })}
      </div>
      {text && participants.map((p) => (
        <Caret key={p.client.id} participant={p} text={text} parentRef={textDisplayRef} fontSize={fontSize} />
      ))}
       {currentTournament.status === 'waiting' && (
        <div className="absolute inset-0 flex items-center justify-center bg-background/80">
          <p className="text-xl text-muted-foreground">Waiting for tournament to start...</p>
        </div>
      )}
