...
export const TournamentRoomOrchestrator = () => {
  ...
  toWatch = participants[member.id] || (Object.values(participants).sort((a, b) => b.correctPosition - a.correctPosition)[0]) || null;
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-slate-100 font-sans overflow-hidden">
      ...
      <MainContentLayout
        statsSlot={
          <StatsPanel toWatch={toWatch} textLength={data.text?.length ?? 0} upcoming={!data.startedAt} />
        }
        participantsSlot={
          <ParticipantsPanel
            toWatch={toWatch}
