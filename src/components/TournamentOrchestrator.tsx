...
export const TournamentRoomOrchestrator = () => {
  ...
  if (
    ...
    if (gamePhase === "error_static_data")
      message = staticError || "Failed to load tournament details.";

    const showLoading =
      gamePhase === "initializing" &&
      (isStaticLoading || authHookResult.isLoading);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-background text-foreground p-4">
        <ErrorMessage
          ...
          message={
            lastSocketError || "Could not connect to the tournament server."
          }
        />
  ...
  const tournamentScheduledFor =
    tournamentSession?.scheduled_for || staticTournamentInfo?.scheduled_start;
