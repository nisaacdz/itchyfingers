...
export default function TypingChallengeArea({ liveSession }: TypingChallengeAreaProps) {
  const getDescription = () => {
    if (liveSession?.started_at && !liveSession?.ended_at) return "The race is on!";
    if (liveSession?.ended_at) return "Race has ended.";
    return "Waiting for the tournament to start...";
  };

  return (
    <Card className="lg:col-span-2"> {/* This class depends on the parent grid */}
      <CardHeader>
