export function toast(type: "success" | "failure", message: string) {
  ...
  if (!toastContainer) {
    ...
    toastContainer.className =
      "fixed bottom-5 left-1/2 transform -translate-x-1/2 flex flex-col items-center space-y-2 pointer-events-none z-50"; // Tailwind classes for positioning and layout
    document.body.appendChild(toastContainer);
  }

  const baseToastClasses =
    "toast px-4 py-2 rounded-md shadow-lg text-white opacity-0 transform translate-y-5 transition-opacity duration-300 ease-out transition-transform duration-300 ease-out pointer-events-auto";
  ...
  const toaster = (message: string, type: "success" | "failure") => {
    if (!message) return;
